...
uint8 arm_uart_flag = 0;    // ARM?????????¦Ë
uint8 arm_uart_flag_on = 0;
uint8 testuart_flag = 0;         // ????????????¦Ë
uint8 data_length = 0;           // ???????
...
void My_Communication_Init(void)
{
    ...
    uart_rx_interrupt(UART_1, 1);                                  // ??????1?????§Ø?
    uart_rx_interrupt(UART_4, 1);                                  // ??????2?????§Ø?
    NVIC_SetPriority(LPUART1_IRQn, 0);                             // ???????1?§Ø??????
    NVIC_SetPriority(LPUART4_IRQn, 1);                             // ???????2?§Ø??????
}
 ...
 * @param ??
 * @return ??
 * @attention 1. ?¨²?????????????1?????????????????????????get_data?????§µ????get_data?????????????
 */
void uart1_rx_interrupt_handler(void)
{
    uart_query_byte(UART_1, &get_data);               // ???????1??????????????????get_data??????
    fifo_write_buffer(&uart_data_fifo, &get_data, 1); // ??get_data?§Ö?????§Õ??????
}
 ...
 * @param ??
 * @return ??
 * @attention  1. ?????????????????????????§Ø????????????0xB7????????????????1??
 *             2. ????1????§Ø?????????????????§¹???????§¹?????¦¶?1-16?????????§¹??????????????transform_counts?????§µ???????2??
 *             3. ????2??????????????????????????¦Â0x98???§Ø???????????????????transform_counts??????????????????????????????0???????????0??????????
 */
void get_uartdata(void)
{
    ...
    if (fifo_data_count != 0)
    {
        ...
        else if (get_states == 1) // ???1
        {
            ...
            {
                transform_counts = fifo_get_data[0]; // ?????????????????????????????1-16???????????transform_counts??????
                fifo_get_data[0] = 0;                // ???????
                get_states = 2;                      // ??????2
            }
            else
