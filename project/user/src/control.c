...
void Pos_PidInit(void)
{
  ...
  //��ǰ
  Pos_turn_pid[0].kp = 0.5;   //0.5��ӦĿ���ٶ�40
  Pos_turn_pid[0].kd = 0.5;   //0.5��ӦĿ���ٶ�40
  //���
...
{
    pid->error = Target - Encoder; //Calculate the deviation //����ƫ��
    
    pid->output = pid->kp * pid->error + pid->kd * (pid->error-pid->lastError); //ԭ����+=���ָ�Ϊ=      2024/3/26
    
    pid->lastError=pid->error;//�����ϴ�ƫ��
...
void Drive_Motor()
{
  float LF_Target,LB_Target,RF_Target,RB_Target; //����������������� Ҫ�����ٶȻ����轫λ�û��������������ת��Ϊ�ٶ�
	loc_err = Err_Handle();
	abs_loc_err = fabsf(Err_Handle());   //ԭ�����������Err_Handle()

   if(abs_loc_err < 1.0)//�趨һ����ֵ��Χ��С�������ֵʱ��Ϊ������Ҫ����,ֻ���ٶȻ���Ŀ���ٶȱջ�
  {
	...
	if(loc_err > 0)       //��֪��Ϊʲô��ת��ʱ�������������С��0,���ﵽʱ��Ҫ����
     Turn_Left_flag = 1;//��ת��־λ
  if(loc_err < 0)
     Turn_Right_flag =1;//��ת��־λ

  if(loc_Finish_flag == 0)//λ�õ���δ���
  {
    Set_Distence_m(abs_loc_err);//�����ٶ�ת�������,��Ҫ����һ�²���
...
//      {
//          encoder_sum[i] += fabsf(encoder[i]);//�������ۼ�ֵ
//      }
//        else
//      {
//          Location_pid_flag = 0;//���ٽ���λ��ʽ������������
//          loc_target[i] = loc_last_target[i];//�����ֱ��Ϊ�ϴδ�������ٶȣ�������
//      }
//    }
    ...
    loc_target[0] = LF_Target* 0.2636719 *PI /100;//ת�����ٶ�
    loc_target[1] = LB_Target* 0.2636719 *PI /100;
    loc_target[2] = RF_Target* 0.2636719 *PI /100;
    loc_target[3] = RB_Target* 0.2636719 *PI /100;//��λΪm/s
  ...
  else if(Turn_Right_flag==1)//��ת
  {
    ...
    loc_target[3] = -fabsf(loc_target[3]);
	  ...
	  loc_last_target[0] = loc_target[0];//��¼���ε�����ٶ�
