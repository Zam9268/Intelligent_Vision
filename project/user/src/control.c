...
float Velocity_KI = 0.1; //???PID???????????????????????????????ß’
float turn_angle;//?????????
int spin;//??????????????
int translation=0;//???????????????????????????
int encoder[4];//???????????
float PID_motor[4];//???pid?????????
int midline[Row];//???????????
float Keep_Bias;//??????????
...
void Read_Encoder(void)
{
  ...
  for(uint8 i=0;i<4;i++)
  {
    Speed[i].now_speed = (encoder[i] * 0.2636719*PI); // 0.2637????????????¶À?????????????
  }
//????????????
...
{
    pid->error = target_speed - pid->now_speed;//?????????????????????
    pid->dError = pid->error - pid->lastError;//????????????????????
    pid->lastError = pid->error ;//????°¿???????????????
    pid->output +=(pid->kp*pid->dError)+(pid->ki*pid->error); //???pi??????
    pid->output_last = pid->output;//????°¿??¶≈?pid???
...
void PID_cale()
{
	for(uint8 i=0;i<4;i++)
  {
    Speed[i].output = increment_pid(&Speed[i],Speed[i].target_speed);//???????????????????????
    PID_motor[i]=Speed[i].output;//???????????pwm??????pid_motor[i]
  }
}
...
void motor_close_control(void)
{
  ...
  if (target_motor[0] > 0) //???1   ??? ????????? ???? (1000/TIMER1_PWM_DUTY_MAX*100)
  {
...
{
  int k = 0;
  translation=midline[Row-1]-Col/2; //?????????ß≥?????????
  for(k = Row; k < 65; k--)
  {
     spin=spin+midline[k];//?????????????????????
...
/****??????????******/
  // V_enco[0] = 0.2636719 * PI * (float)encoder[0]; // 0.2637????????????¶À
