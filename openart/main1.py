...
#设置模型路径
face_detect = 'yolo3_iou_smartcar_final_with_post_processing_5_31.tflite'
#载入模型
...
while(True):
    clock.tick()
    red.on()
    sensor.set_hmirror(change_flag)
    img = sensor.snapshot()
    #部分图像点的坐标测试
    #while(1):
    ...
    if(change_flag==0):
        ...
        for obj in tf.detect(net,img):
            ...
            if(scores>0.70):
                ...
                my_new_perspective=np.array([[-1.408363e-07, 4.684143, -156.3491], [-2.41688, -0.4475705, 102.4041], [9.81242e-10, -0.00511509, 1]])
                result_matrix=my_new_perspective*point
                finnal_matrix = np.array([[result_matrix[0][0]/result_matrix[2][0]], [result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y], [1]])
                if(change_zhen==1):
                    finnal_matrix[0][0]=-finnal_matrix[0][0]
                my_tabel=[]#创建待发送列表
                if(finnal_matrix[0][0]>0):
                    my_tabel.append(1)#第一位表示x坐标的正负，1为正
                    overflow_level=int(finnal_matrix[0][0])//256
                    ...
                    #判断x坐标的溢出等级
                    overflow_level=int(x_zheng)//256
                    my_tabel.append(overflow_level)
        ...
        for obj in tf.detect(net,img):
            ...
            if(scores>0.70):
                ...
                if(finnal_matrix[0][0]>0):
                    my_tabel.append(0)#第一位表示x坐标的正负，1为正
                    my_tabel.append(int(finnal_matrix[0][0])//256)#表示x坐标不存在溢出
                    #在添加x坐标的时候要判断是否存在溢出,前面已经判断了，后面就存入对应的取余后的坐标
                    my_tabel.append(int(finnal_matrix[0][0]))#发送对应存储的x坐标（注意是取余后）
                    #计算对应y坐标的溢出等级，0：无溢出，1：溢出一个256，后面以此类推，一般都不会超过1024个单位
                    overflow_level = int(finnal_matrix[1][0]) // 256
