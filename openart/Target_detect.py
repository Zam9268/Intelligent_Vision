    ...
    for obj in tf.detect(net,img):
        ...
        if(scores>0.70):
            ...
            if(result_matrix[0][0]>0):
                ...
                if(int(result_matrix[0][0]/result_matrix[2][0])>=256):
                    my_tabel.append(1)#发送1表示x坐标存在溢出，否则不存在溢出
                else:
                    my_tabel.append(0)#表示x坐标不存在溢出
                #在添加x坐标的时候要判断是否存在溢出,前面已经判断了，后面就存入对应的取余后的坐标
                my_tabel.append(int(result_matrix[0][0]/result_matrix[2][0]))#发送对应存储的x坐标（注意是取余后）
                #计算对应y坐标的溢出等级，0：无溢出，1：溢出一个256，后面以此类推，一般都不会超过1024个单位
                if(int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y)>=256 and int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y)<=511):
                    my_tabel.append(1)#y坐标存在溢出
                elif(int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y)<=255):
                    my_tabel.append(0)#y坐标不存在溢出
                elif(int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y)>=512 and int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y)<=767):
                    my_tabel.append(2)
                elif(int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y)>=768 and int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y)<=1023):
                    my_tabel.append(3)
                else:
                    my_tabel.append(4)
                my_tabel.append(int(result_matrix[1][0]/result_matrix[2][0]+fixed_deta_y))#发送取余后的坐标
                send_data[my_tabel,5]
                break;
            else:#对x坐标判断是否为负
                my_tabel.append(0)#x坐标为负数
                x_zheng=-int(result_matrix[0][0]/result_matrix[2][0]);#对x坐标取正
                #判断x坐标的溢出等级
